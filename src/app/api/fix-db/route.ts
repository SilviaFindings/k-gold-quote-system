...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn('⚠️ price_history.id 修改失败或已存在:', e.message);
    }

    // 步骤3: 修改 products.id 字段
    ...
    try {
      ...
      console.warn('⚠️ products.id 修改失败或已存在:', e.message);
    }

    // 步骤4: 修改 price_history.product_id 字段
    ...
    try {
      ...
      console.warn('⚠️ price_history.product_id 修改失败或已存在:', e.message);
    }
    ...
    const isFixed = productsIdLength >= 100 && priceHistoryIdLength >= 100 && priceHistoryProductIdLength >= 100;
